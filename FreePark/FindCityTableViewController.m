...
#import "FindCityTableViewController.h"
#import "Item+CoreData.h"
#import "AppDelegate.h"
#import "ParkPanaDatabaseAPI.h"
#import "MainMapViewController.h"

@interface FindCityTableViewController ()

@end

@implementation FindCityTableViewController

- (void)viewDidLoad {
    
    [super viewDidLoad];

    [ParkPanaDatabaseAPI GetCityInfoList:^(NSMutableArray *cityInfos) {
        [self initializeTableContent:cityInfos];
...
- (void)initializeTableContent:(NSMutableArray*) cityInfos {
    
    self.mCityInfos = [cityInfos mutableCopy];
    self.managedObjectContext = [[NSMutableArray alloc] init];
    for( NSObject* obj in self.mCityInfos )
    {
        CityInfo *cityInfo = (CityInfo *)obj;
        [self.managedObjectContext addObject:cityInfo.name];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CityInfo *cityInfo = [self.mCityInfos objectAtIndex:indexPath.section];
    [((MainMapViewController *)[[AppDelegate sharedDelegate] mainMapViewController]) setCityInfo:cityInfo];
    [((MainMapViewController *)[[AppDelegate sharedDelegate] mainMapViewController]) showParking];
